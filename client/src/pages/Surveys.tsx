...
export default function Surveys() {
  ...
  const onArchive = async (id: string) => {
    await axios.patch(`/api/surveys/${id}/archive`);
    await load();
  };

  const onUnarchive = async (id: string) => {
    await axios.patch(`/api/surveys/${id}/unarchive`);
    await load();
  };
      ...
      <div className="surveys-grid">
        <section className="mm-card mm-card--pad">
          ...
          <div className="mm-tabs">
            <button
              className={"mm-tab " + (tab === "active" ? "is-active" : "")}
            ...
            <button
              className={"mm-tab " + (tab === "archived" ? "is-active" : "")}
                      ...
                      <td className="mm-muted">
                        {Array.isArray(s.departments) && s.departments.length
                          ? s.departments.slice(0, 2).join(", ") +
                            (s.departments.length > 2 ? `, +${s.departments.length - 2}` : "")
                          : "—"}
                      </td>
                      <td>
        ...
        <aside className="mm-card mm-card--pad">
          ...
          <div className="quick">
            <button className="quick-item" onClick={() => navigate("/app/surveys/create")}>
              ...
              <span>
                <span className="quick-title">Создать опрос</span>
                <span className="quick-sub">Собери форму из шаблона</span>
              </span>
            </button>

            <button className="quick-item" type="button" onClick={() => navigate("/app/surveys/create")}>
              <span className="quick-ico">
                <Wand2 size={18} />
              </span>
              <span>
                <span className="quick-title">Использовать шаблон</span>
                <span className="quick-sub">Готовые наборы вопросов</span>
              </span>
            </button>
            ...
            <button className="quick-item" type="button">
              ...
              <span>
                <span className="quick-title">Просмотреть аналитику</span>
                <span className="quick-sub">Статистика по отделам</span>
              </span>
            </button>

            <div className="mm-dashedbox">
              <div className="mm-dashed-title">Создайте свой первый опрос</div>
              <div className="mm-muted" style={{ marginTop: 6 }}>
                Начните собирать обратную связь от команд прямо сейчас
              </div>
              ...
              <div className="mm-dashed-actions">
                ...
                <button className="mm-btn mm-btn--light" type="button">
                  Шаблоны вопросов →
                </button>
              </div>
            </div>
