...
type Survey = {
  ...
  questions?: Question[]; // на всякий случай
};
...
export default function SurveyDetails() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const qs: Question[] = res.data?.template?.questions ?? res.data?.questions ?? [];
  ...
  const questions = useMemo(() => survey?.template?.questions ?? survey?.questions ?? [], [survey]);
    ...
    <div className="page survey-details">
      ...
      <div className="card">
        ...
        {!survey || questions.length === 0 ? (
          <div className="empty">
            <b>В этом опросе нет вопросов.</b>
          </div>
        ) : (
