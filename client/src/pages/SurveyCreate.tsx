...
export default function SurveyCreate(){
  ...
  const [allDepts]=useState<string[]>([
    "Разработка","Маркетинг","Продажи","Поддержка клиентов","Аналитика и данные","HR","Финансы","Операции",
  ]);
  ...
  const onCreate=async()=>{
    ...
    try{
      ...
      setError(e?.response?.data?.error||"Ошибка создания опроса");
    } finally{
      setSaving(false);
    }
  };
      ...
      <div className="page-header">
        ...
        <div className="page-actions">
          ...
          <button className="btn btn-primary" type="button" onClick={onCreate}
            ...
            <Save size={18}/>{saving?"Создаём...":"Создать"}
          </button>
        </div>
      </div>
        ...
        <section className="card">
          <div className="form-stack">
            ...
            <div>
              ...
              <input className="field" type="number" min={3} max={50}
                value={threshold} onChange={(e)=>setThreshold(Number(e.target.value))}/>
              <div className="help">Совет: для корректной анонимности обычно ставят <b>7+</b> ответов на отдел.</div>
            </div>
          </div>
        </section>
